...
// Updated utility function to fetch video from noobs-api
async function fetchVideoData(videoUrl) {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 30000); // 30 second timeout for video
  
  try {
    const videoId = extractYouTubeId(videoUrl);
    const apiUrl = `${BASE_URL}/dipto/ytDl3?link=${encodeURIComponent(videoId)}&format=mp4`;
...
function formatVideoInfo(videoInfo, videoUrl) {
...
async function preloadVideo(session) {
  ...
  try {
    ...
    const videoResponse = await fetch(session.downloadUrl, {
      headers: {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36',
...
const video = async (message, client) => {
  try {
    ...
    if (command === "video2") {
      ...
      if (args.length === 0 || !args.join(" ")) {
        ...
        return await client.sendMessage(message.from, {
          text: toFancyFont("Please provide a video name or YouTube URL to download"),
      ...
      try {
        ...
        // Fetch video data from noobs-api
        ...
        const videoInfoText = formatVideoInfo(videoInfo, videoUrl);
        ...
        // Start preloading video in background (non-blocking)
        setTimeout(() => preloadVideo(sessionData), 100);
        ...
        const buttons = [
          ...
          {
            buttonId: `${prefix}sd`,
            buttonText: { displayText: "📹 ❯❯ sᴅ ᴠɪᴅᴇᴏ" },
            type: 1
          },
          {
    ...
    } else if (command === "hd" || command === "sd" || command === "document") {
      ...
      try {
        ...
        if (command === "hd" || command === "sd") {
          // Send as video message
          await client.sendMessage(message.from, {
            video: videoData,
            mimetype: 'video/mp4',
            caption: `🎬 *${session.videoTitle}*\n\n> 📥 Downloaded by CASEYRHODES AI🧑‍💻`,
            fileName: `${session.videoTitle.replace(/[^\w\s]/gi, '')}.mp4`.substring(0, 50) || 'video.mp4',
            contextInfo: newsletterContext
          }, { quoted: message });
